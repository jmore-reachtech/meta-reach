...
do_configure_append () {
	mv ${S}/conf/ifplugd.action ${S}/conf/ifplugd.action.orig
	cp ${WORKDIR}/ifplugd.action ${S}/conf/ifplugd.action
}

do_install_append () {
	install -m 0755 ${WORKDIR}/netservices ${D}${sysconfdir}/ifplugd/
}
