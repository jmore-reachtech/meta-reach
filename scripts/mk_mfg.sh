...
if [ ! -f $SD_ROOTFS_IMAGE ]; then
	echo "SD_ROOTFS_IMAGE $ROOTFS_IMAGE not found: $SD_ROOTFS_IMAGE " 
	exit 1
fi

SD_ROOTFS_UBIFS=$IMAGE_DIR/$SD_IMAGE-$MACHINE.ubifs
if [ ! -f $SD_ROOTFS_UBIFSE ]; then
	echo "SD_ROOTFS_UBIFS $ROOTFS_UBIFS not found: $SD_ROOTFS_UBIFSE"
	exit 1
fi

SD_BOOT_IMAGE=$IMAGE_DIR/$SD_IMAGE-$MACHINE.linux.sb
if [ ! -f $SD_BOOT_IMAGE ]; then
	echo "SD_BOOT_IMAGE $BOOT_IMAGE not found: $SD_BOOT_IMAGE"
	exit 1
fi

SD_BOOT_IMAGE_HEADER=$IMAGE_DIR/$SD_IMAGE-$MACHINE.sb.header
if [ ! -f $SD_BOOT_IMAGE_HEADER ]; then
	echo "SD_BOOT_IMAGE_HEADER $BOOT_IMAGE_HEADER not found: $SD_BOOT_IMAGE_HEADER"
	exit 1
fi
...
if [ ! -f $FLASH_ROOTFS_IMAGE ]; then
	echo "FLASH_ROOTFS_IMAGE $ROOTFS_IMAGE not found: $FLASH_ROOTFS_IMAGE"
	exit 1
fi

FLASH_ROOTFS_UBIFS=$IMAGE_DIR/$FLASH_IMAGE-$MACHINE.ubifs
if [ ! -f $FLASH_ROOTFS_UBIFSE ]; then
	echo "FLASH_ROOTFS_UBIFS $ROOTFS_UBIFS not found: $FLASH_ROOTFS_UBIFSE"
	exit 1
fi

FLASH_BOOT_IMAGE=$IMAGE_DIR/$FLASH_IMAGE-$MACHINE.linux.sb
if [ ! -f $FLASH_BOOT_IMAGE ]; then
	echo "FLASH_BOOT_IMAGE $BOOT_IMAGE not found: $FLASH_BOOT_IMAGE"
	exit 1
fi

FLASH_BOOT_IMAGE_HEADER=$IMAGE_DIR/$FLASH_IMAGE-$MACHINE.sb.header
if [ ! -f $FLASH_BOOT_IMAGE_HEADER ]; then
	echo "FLASH_BOOT_IMAGE_HEADER $BOOT_IMAGE_HEADER not found: $FLASH_BOOT_IMAGE_HEADER"
	exit 1
fi
